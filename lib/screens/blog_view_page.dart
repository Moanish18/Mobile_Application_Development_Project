...
// BlogViewPage allows viewing a specific blog post and using text-to-speech to read the content
class BlogViewPage extends StatefulWidget {
  final String postId; // ID of the blog post to be viewed

  BlogViewPage({required this.postId}); // Constructor requiring postId
...
class _BlogViewPageState extends State<BlogViewPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('Blog Post'), // App bar title
      ...
      body: FutureBuilder<DocumentSnapshot>(
        future: fbService.readPost(widget.postId), // Fetch the blog post
        builder: (context, snapshot) {
          ...
          if (!snapshot.hasData || !snapshot.data!.exists) {
            return Center(child: Text('Post not found.')); // Show message if post is not found
          }
  ...
  List<Widget> _buildPostContent(Map<String, dynamic> post, BuildContext context) {
    return [
      ...
      SizedBox(height: 16),
      Text(post['content']), // Display post content
    ];
  }
